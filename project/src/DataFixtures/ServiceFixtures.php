...
class ServiceFixtures extends BaseFixture implements DependentFixtureInterface
{
    ...
    protected function loadData(ObjectManager $manager)
    {
        $this->createEntity(Service::class, 1, function (Service $service) {
            ...
            $service
                ->setTitle('Клининг')
                ->setSubTitle('ПРОФЕССИОНАЛЬНАЯ УБОРКА ДЛЯ ВАШЕГО БИЗНЕСА')
                ->setDescription('<p>Наши услуги — это ключ к безупречной чистоте и комфорту. Мы создаем и поддерживаем пространства, в которых каждая деталь сияет чистотой. Благодаря передовым методам и инновационным технологиям</p>')
